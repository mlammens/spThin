...
The `spThin` package contains functions to spatially thin and filter occurrence records. Here we provide a tutorial on this package can be used to process datasets for environmental niche modelling.
...
# Filter data
Spatial filtering is another process for reducing bias. This process works by overlaying a grid over the observation records and randomly selecting a single record in each grid cell. Generally, this method can be used on much larger datasets than spatial thinning.
...
Now, we can spatially filter the dataset. Here, we will generate 100 replicates using 180km grid cells.
...
Thinned or filtered datasets using the `[[` method. The thinned or filtered datasets are stored as `SpatialPoints` or `SpatialPointsDataFrame` objects. Here, we will retrieve the first filtered replicate and store it in an object.
...
# Save processed data
Finally, the thinned and filtered datasets can be saved using the `write` method. Here, we will saved the filtered datasets to a temporary directory.
